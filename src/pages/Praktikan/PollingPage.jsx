...
import PollingHeader from "../../components/Components/Praktikans/Sections/PollingHeader";
import PollingContent from "../../components/Components/Praktikans/Sections/PollingContent";
...
export default function PollingPage() {
    ...
    const categoriesQuery = useQuery({
        ...
        queryFn: async () => {
            const { data } = await api.get("/api/jenis-polling");
            return data ?? {};
        },
    });
    ...
    useEffect(() => {
        if (categories.length > 0) {
            ...
            if (available.length === 0 && categories.length > 0) {
                ...
                setAllCategoriesSubmitted(false);
            }
        } else {
            setAvailableCategories([]);
            setAllCategoriesSubmitted(false);
    ...
    const asistenQuery = useQuery({
        ...
        queryFn: async () => {
            const { data } = await api.get("/api/polling-assistants");
            if (data?.success) {
                return data.asisten ?? [];
            }
    ...
    const handleSubmitAll = async () => {
        ...
        try {
            ...
            const { data } = await api.post("/api/pollings", submissions);
